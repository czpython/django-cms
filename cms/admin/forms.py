...
class PluginAddValidationForm(forms.Form):
    ...
    def clean(self):
        ...
        if parent_plugin:
            ...
            parent_sibling = parent_plugin.get_right_sibling()

            if parent_sibling and position > parent_sibling.position:
                message = ugettext("Plugin position must be less than or equal to %(position)d")
                self.add_error('placeholder_id', message % {'position': parent_sibling.position})
                return self.cleaned_data
