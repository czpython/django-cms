...
class Placeholder(models.Model):
    ...
    def add_plugin(self, instance):
        ...
        # If the distance between the last plugin and the new one
        # is higher than one, there's no need to recalculate the tree
        ...
        self.mark_as_dirty(instance.language, clear_cache=False)
        return instance
    ...
    def move_plugin(self, plugin, target_position, target_placeholder=None, target_plugin=None):
        ...
        # Squash the positions by using ROW_NUMBER starting with 1
    ...
    def _move_plugin_to_placeholder(self, plugin, target_position, target_placeholder, target_plugin=None):
        ...
        if target_last_plugin:
            target_last_plugin = target_placeholder.get_last_plugin(plugin.language)
            source_offset = max(target_last_plugin.position, source_last_plugin.position)
